...
with open(os.path.join(os.path.dirname(__file__), 'xml', 'word_line.xml'), 'r') as f:
...
with open(os.path.join(os.path.dirname(__file__), 'xml', 'word_line_sect2.xml'), 'r') as f:
...
def _convert_to_pdf_win(filepath: str):
...
def convert_to_pdf_win(filepath: str):
    hostname = 'http://innostan.ddns.net:8085'
    # hostname = 'http://192.168.0.122:8000'
    with open(filepath, 'rb') as f:
        files = {'filecontent': f}
        r = requests.post(f'{hostname}/pdfgen/', files=files)
        result = json.loads(r.content)
    if result['content']:
        pdf_content = base64.b64decode(result['content'])
        target_path = filepath.replace('.docx', r'.pdf')
        with open(target_path, 'wb') as result_f:
            result_f.write(pdf_content)
