-- FUNCTION list_filter_events_later_than(bigint,bigint,bigint,bigint,bigint,timestamp with time zone,timestamp with time zone,bigint[],varchar[],bigint[],varchar[], boolean);

DROP FUNCTION list_filter_events_later_than(bigint,bigint,bigint,bigint,bigint,timestamp with time zone,timestamp with time zone,bigint[],varchar[],bigint[],varchar[], boolean);

CREATE OR REPLACE FUNCTION list_filter_events_later_than(
...
begin
  ...
  if char_length(query_filter) = 0 then
        ...
        J.result,
				...
				order by J.id asc limit max_count_in;
  else
     query_text := 'select J.id as msg_id,
				...
				where ' || query_filter || '
				order by J.id asc limit ' || quote_nullable(max_count_in) || ';';
