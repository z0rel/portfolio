...
, m_MoverController()
{
	...
	CreateScene(); /// Создание объектов для отображения
...
void GLWidget::initializeGL()
{
	...
	m_GlView.initGl(); /// Инициализация OpenGL
...
void GLWidget::paintGL()
{
	...
	glLoadIdentity(); /// Загрузка матрицы идентичности

	/// ---- Определения GLC ----
	m_GlView.setDistMinAndMax(m_World.boundingBox()); /// Вычисление глубины камеры представления.

	m_Light.enable();    /// Определение света.
	m_Light.glExecute();
...
void GLWidget::CreateScene()
{
	...
	//QFile fileEx07(fileEx07_inf.absoluteFilePath());
