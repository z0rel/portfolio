...
class GulpSSH extends EventEmitter {
  ...
  getClient() {
    ...
    if (privateKeyFile) {
      if (
        privateKeyFile.charAt() === '~' &&
        (path.sep === '\\' ? /\/|\\/.test(privateKeyFile.charAt(1)) : privateKeyFile.charAt(1) === '/')
      ) {
      ...
      function (file, encoding, callback) {
        ...
        getSftp(function (err, sftp) {
          ...
          internalMkDirs(sftp, outPath, function (err) {
            ...
            write.on('error', done).on('finish', done);
